...
twelve articles of the withdrawal-strategies part. Nothing here is implemented.
Delete this file once the retained figures are shipped (docs/ stays curated).

The review itself is done and shipped; this is only the illustration half of it.
The figures, when built, follow the v2 plate system (`figures_v2.go`) and the
data-backed pattern recorded in `fire-book-design.md` (frozen literal arrays in
the plate, a guard test recomputing them from the engine).
...
**2.1 La falaise silencieuse — A (prioritaire)**
...
**6.1 La double contracyclicité — A (prioritaire)**
...
**7.1 Deux thermomètres sur le même plan — B (prioritaire)**
...
**10.1 Les crédits de mortalité par âge — A (prioritaire)**
...
**12.1 L'arbre de décision en cinq étapes — A**
...
## Si je devais n'en faire que cinq
...
Les cinq sont indépendantes et se font dans n'importe quel ordre. Trois sont
en coût A.
