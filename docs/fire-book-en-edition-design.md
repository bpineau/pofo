...
| suivre-inflation | tracking-inflation | adapt (light) | The index section rewritten for the US reader, same slot and length: CPI-U (BLS, the headline index, the not-seasonally-adjusted version TIPS follow), CPI-W (the COLA index, the third-quarter rule, the 2.8% raise paid in 2026), PCE (the Fed's 2% target, useless for indexing a withdrawal) and the CPI-E research index (+0.2 points a year over CPI-W 1982-2014, gap almost gone 2002-2021) replace IPC/IPCH. The blind spot changed nature and kept the argument: the US does impute owner housing, so the hole is that shelter enters as owners' equivalent rent while house prices, mortgage interest and property taxes left the index in 1983 (rental equivalence), plus the OER lag; the paid-off retiree against the would-be buyer survives unchanged. "Where to read them" became bls.gov and FRED (INSEE and Eurostat dropped, and with them the INSEE personal-inflation simulator: the evening's calculation now crosses statements with the BLS category indexes on FRED). The health line took a US number, the Part B premium (about 5.5% a year over two decades, $202.90 for 2026, up 9.7%), with a dated-accuracy caveat; the breakeven took the 10-year TIPS breakeven (about 2.3% in mid-2026, FRED) with the same caveat. Sources: bls.gov and ssa.gov are bot-gated (403), so the COLA rule, the CPI-E gap, the shelter and OER weights, the 1983 rental-equivalence switch, the Part B premium and the 10-year breakeven were verified through search results citing the primary sources (Federal Register, CRS, CBO, FRED) rather than fetched directly. Generalized: the `^HICP-FR` deflator symbol dropped (the sentence says "the deflated series a simulator runs on"); OATi/OAT€i and the ECB target dropped with their indexes. One fr-only link dropped, [[sante-et-protection-sociale]] (health care and long-term care now point at [[spending-in-retirement]] alone); no us-* pointer needed. The plates stay single-source: 31 dictionary entries added for `ecart-compose` and `panier-contributions`, with "l'IPCH : 2,1 %" rendered as "the official index: 2.1%" and the caption of `panier-contributions` saying in one clause that the household budget is in euros. `figure-audit.sh en` green. |
| inflation-et-taux-de-retrait | inflation-and-withdrawal-rates | generalize | The indexation-inventory table, the article's central tool, rewritten in US category terms: "Pension legale" becomes Social Security (COLA-indexed by law, the raise politically argued over), "Rentes privees" becomes private pensions and annuities (corporate plans almost never index, an inflation-linked annuity costs a deep cut in starting income), "Linkers / echelle indexee" becomes TIPS and an indexed ladder, "fonds euros" becomes stable value in the asset row and "stable value and the aggregate" in the worked example, and the fixed-rate credit line becomes a fixed-rate mortgage. The reading paragraph under the table drops "un plan francais type" for "the typical plan", with Social Security as the source of the long-inflation tail. The conditional-numbers callout drops IPCH for "the CPI". Four fr-only links dropped: two [[inflation-histoire]] (the regimes aside in mechanism 2, and the "In this book" bullet, both neutralized into plain prose) and [[retraite-legale]] (replaced by the one allowed pointer to [[us-healthcare-and-social-security]] on the Social Security row). [[la-machine-pofo]] maps to [[under-the-hood]] and the sentence names no tool. 16 dictionary entries added for `inflation-episode-derive`; `figure-audit.sh en` green. |
| se-proteger-de-inflation | inflation-protection | generalize | Tier 1 rebuilt around the US analogue: "la pension legale, et son extension, le rachat de trimestres" becomes the public pension and delaying your claim (same argument, subsidized indexed income for life, political haircut kept), with the one allowed pointer to [[us-healthcare-and-social-security]]; the deductibility aside becomes "the rules where you live decide the exact price of waiting". The rent row drops the French rent index and the 2022-2023 cap for "they follow prices with a lag, and rent control can cap them". The blacklist opens on "stable value funds and money market funds" instead of fonds euros et monetaire. In the Sonia example the eight-quarter buyback becomes a two-year delayed claim and "le fonds euros excedentaire" becomes "the excess stable value and cash"; the French payout rate is dropped, EUR amounts kept. `spendDrift +0,3 a +0,5 %` becomes "a real spending drift of 0.3 to 0.5 points a year", the tool unnamed. Three fr-only links dropped: two [[inflation-histoire]] (the opening enemy clause and the rent regulatory aside, both plain prose now) and [[retraite-legale]] (twice: the Tier 1 heading line and the "Going further" building-blocks bullet). 12 dictionary entries added for `actions-rattrapent`; `figure-audit.sh en` green. |
| hyperinflation-et-extremes | hyperinflation-and-extremes | generalize | Historical episodes all kept as evidence (Weimar, Hungary, Yugoslavia, Zimbabwe, Venezuela, Argentina, Turkey, Israel, France 1945-1948, the US gold ban, French exchange controls, Cyprus 2013, Greece 2015). The euro-area redenomination aside becomes "every monetary arrangement carries its own version of the tail", with the currency-union case kept and a dollar clause added (the dollar cannot be redenominated, only debased). "80-90 % du patrimoine hors juridiction francaise" is corrected for a dollar investor: a cap-weighted world fund is still roughly two thirds American, so the ex-US share is a decision rather than a byproduct. UCITS domiciled in Ireland or Luxembourg becomes segregated fund assets held by a custodian, with the honest caveat that the separation is legal and not jurisdictional. "100 k euros depots, 70 k euros assurance-vie" becomes deposit insurance where you bank, with the FDIC ceiling in one clause ($250,000 per depositor, per insured bank, per ownership category). "Un compte-titres dans une deuxieme juridiction europeenne" becomes a foreign brokerage account; "une poche de devises fortes hors euro" becomes hard currency other than your own. In the Sonia extract, Irish UCITS at a French and a German broker become index funds at two brokers in two countries, and the fonds euros at two insurers become two banks under the insurance ceiling plus a money market fund. Four fr-only links dropped: two [[inflation-histoire]] (opening sentence and the real-estate clause, both plain prose now, plus the "Going further" bullet), [[taxe-puma]] (now "a tax code that leans a little harder on people living off capital") and [[enveloppes-francaises]] (now account types and legal statuses, with the one allowed pointer to [[us-accounts-and-account-order]]). 11 dictionary entries added for `weimar-reel`; `figure-audit.sh en` green. |
| psychologie-du-retrait | the-psychology-of-spending | generalize | Locale-neutral throughout: "la generosite differee jusqu'au notaire" becomes generosity postponed until the estate is settled; "la pension legale" becomes "any pension that pays for life"; the mental-accounting example keeps a generic checking account and brokerage account. The Blanchett-Finke result is quoted in dollars (a US study, dollar figures in the source), while the book's own worked case stays in euros (EUR 1.7M, EUR 58,000). No US-specific pointer was needed, so none was added. Two fr-only links dropped: [[succession-et-transmission]] (the sentence now reads "the help to your children at the moment it would change their lives rather than after you are gone") and [[retraite-legale]] (permission is now half the value of an annuity and of any lifetime pension). 28 dictionary entries added for `mourir-riche`; `figure-audit.sh en` green. |
| temoignages-fire | voices-from-real-retirees | generalize | The quoted corpus is already anglophone, so the pass was light. The French-reader asides go: "et cote francais les fils au long cours des forums patrimoniaux" is dropped from the opening (the forums are already named later), "le lecteur francais a plan moyen transpose" becomes "a reader with an ordinary plan transposes, never copies", and the r/vosfinances link plus the sentence on the smaller French community are dropped from "The corpus, and where to read it". Two fr-only links dropped there ([[taxe-puma]], [[retraite-legale]]); the sentence now reads "wherever you live, the local rules are the one thing no foreign account can settle for you" and carries the single pointer [[us-healthcare-and-social-security]]. English titles restored rather than back-translated (LivingAFI "The Drawdown", the "annual updates", "one more year", the retirement police). No figure block in this article, so no dictionary entry; `figure-audit.sh en` run all the same. |
