...
| `us-total-market-reference-design.md` | the US total-market reference `USMKT-USD` (Ken French market factor, daily 1926-07) and the VTI backcast built on the target's own share class: why a 0.99 correlation to the S&P 500 does not make it a stand-in, the two mandatory pre-write validations, the measured grossness constant, and the audit gain |
...
| `fire-envelopes-tax-model-design.md` | the per-envelope tax question, measured and closed (2026-09-10): what `Tax`/`Envelope` already do, the calibration recipe for the single blended rate (gain-weighted rate, capital-weighted gain fraction) with its worked example, why the explicit CTO/PEA/AV/PEE model is worth 0.015 point of withdrawal rate against 0.30 for the embedded gain fraction, and the PEE break-even match formula |
...
| `index-benchmarks-design.md` | why `MSCIWORLD`/`SP500` are fee-free long-history index benchmarks with bare ids and no SIM variant; the tail policy and validation bands that keep the three MSCI monthly anchors current (`make msci-refdata`) |
