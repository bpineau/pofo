...
.PHONY: usmkt-refdata
usmkt-refdata: ## (Re)generate USMKT-USD, the CRSP total-market factor behind the total-US-market backcast (network); validates against SP500-USD, so run after `make sp500-refdata` and `make simdata` after
	$(GO) run ./cmd/gen-usmkt-refdata
	$(GO) build -o pofo ./cmd/pofo
