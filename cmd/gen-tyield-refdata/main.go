// Command gen-tyield-refdata builds TREASURY-LONG-YIELD, the bundled US LONG
// Treasury par-yield history the zero-coupon STRIPS reconstruction is priced
// off (pkg/simgen.TreasuryZeroTR, behind the ZROZ recipe). It runs at
// data-generation time only (network); the pofo binary embeds the CSV and never
// fetches the Federal Reserve.
//
// Why a yield series rather than one more total-return series. Everything long
// the repository already bundles is a COUPON bond reconstruction
// (TREASURY-LONG-USD, a 20-year par bond), and a coupon bond's duration
...
// than constants frozen into a CSV.
//
// One file is written into pkg/datasets/refdata/:
//
...
//
// It is assembled from the Federal Reserve's H.15 selected interest rates, read
// through the DBnomics mirror (free, key-less), in three segments:
//
...
//
// The map is affine, y30 = a + b·y20, fitted by least squares on the 10 702
// days the two points overlap, then shifted so the mapped series MEETS the
// spine exactly at the junction (the same construction the euro-area long
...
// absence of a frozen fetch, freshness, and a cross-check of the 20-year par
// reconstruction this series implies against the already-bundled
// TREASURY-LONG-USD. Nothing here is trusted on the strength of having
// downloaded cleanly.
//
...
const (
	...
	// outID is the bundled series this generator owns.
	outID = "TREASURY-LONG-YIELD"
	// The three H.15 series read, all of them "U.S. government securities /
...
func report(id string, pts []marketdata.Point) {
	...
	log.Printf("%-20s %6d points  %s..%s  range %.2f..%.2f %%/yr", id, len(pts),
		pts[0].Date.Format("2006-01-02"), pts[len(pts)-1].Date.Format("2006-01-02"), lo, hi)
}
