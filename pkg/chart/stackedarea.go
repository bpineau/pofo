...
func StackedArea(opt Options, xLabel, yLabel string, series []AreaSeries) string {
	...
	for i := range series {
		fmt.Fprintf(&b, `<polygon points="%s" fill="%s" fill-opacity="0.55" stroke="#FFFDF9" stroke-width="1"/>`+"\n",
	...
	for v := 0.0; v <= vmax+ystep/1e6; v += ystep {
		y := yAt(v)
		fmt.Fprintf(&b, `<line x1="%g" y1="%.1f" x2="%g" y2="%.1f" stroke="#EAE1D3" stroke-opacity="0.6"/>`+"\n", x0, y, x1, y)
