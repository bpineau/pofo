...
func HBars(opt Options, bars []Bar) string {
	...
	if opt.Title != "" {
		fmt.Fprintf(&sb, `<text x="8" y="22" font-size="14" font-weight="600" fill="#222">%s</text>`+"\n", esc(opt.Title))
	}
	if len(bars) == 0 {
		sb.WriteString("</svg>")
		return sb.String()
	...
	// Zero axis.
	fmt.Fprintf(&sb, `<line x1="%.1f" y1="%g" x2="%.1f" y2="%.1f" stroke="#999"/>`+"\n", zero, padT-4, zero, float64(h)-padB)
	...
	for i, b := range bars {
		...
		color := "#2ca02c" // negative: reduces ruin (good)
		anchor, tx := "start", x+4
		if b.Value > 0 {
			color = "#d62728" // positive: increases ruin
		} else {
			anchor, tx = "end", x-4
		}
		fmt.Fprintf(&sb, `<rect x="%.1f" y="%.1f" width="%.1f" height="%.1f" rx="2" fill="%s"/>`+"\n",
			left, cy-barH/2, width, barH, color)
		fmt.Fprintf(&sb, `<text x="%.1f" y="%.1f" dy="0.35em" font-size="11" fill="#555" text-anchor="end">%s</text>`+"\n",
			x0-8, cy, esc(b.Label))
		if b.Text != "" {
			fmt.Fprintf(&sb, `<text x="%.1f" y="%.1f" dy="0.35em" font-size="11" fill="#333" text-anchor="%s">%s</text>`+"\n",
