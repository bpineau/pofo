...
func Fan(opt Options, xLabel string, bands [][]float64, samples [][]float64) string {
	...
	for _, i := range axisTicks(steps) {
		...
		fmt.Fprintf(&b, `<text x="%.1f" y="%.1f" font-size="11" fill="#555" text-anchor="middle">%d</text>`+"\n", xAt(i), y1+16, i)
	}
	if xLabel != "" {
	...
	// Individual sample paths (clipped to the band range), ruin ones in red.
	for _, s := range samples {
		color := "#8893a5"
		...
		fmt.Fprintf(&b, `<path d="%s" fill="none" stroke="%s" stroke-width="1" stroke-opacity="0.7"/>`+"\n",
			linePath(clipped, xAt, yAt), color)
	}

	// Median line: the central band when the percentile count is odd.
	if n%2 == 1 {
		fmt.Fprintf(&b, `<path d="%s" fill="none" stroke="#10243f" stroke-width="2.2" stroke-linejoin="round"/>`+"\n",
