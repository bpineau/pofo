...
func Fan(opt Options, xLabel string, bands [][]float64, samples [][]float64) string {
	...
	for _, i := range axisTicks(steps) {
		...
		fmt.Fprintf(&b, `<text x="%.1f" y="%.1f" font-size="12" fill="#6E6157" text-anchor="middle">%d</text>`+"\n", xAt(i), y1+16, i)
	}
	if xLabel != "" {
		fmt.Fprintf(&b, `<text x="%.1f" y="%.1f" font-size="12" fill="#6E6157" text-anchor="middle">%s</text>`+"\n", (x0+x1)/2, y1+32, esc(xLabel))
	}
	...
	fmt.Fprintf(&b, `<line x1="%g" y1="%g" x2="%g" y2="%g" stroke="#C0492F" stroke-width="1.8"/>`+"\n", x0, y1, x1, y1)
	fmt.Fprintf(&b, `<text x="%g" y="%g" font-size="11" font-weight="600" fill="#C0492F">ruin · 0</text>`+"\n", x0+5, y1-4)
	fmt.Fprintf(&b, `<line x1="%g" y1="%g" x2="%g" y2="%g" stroke="#CBBFAE"/>`+"\n", x0, y0, x0, y1)

	// Shaded bands, outermost first so inner pairs paint on top.
	const bandFill = "#2E6E63"
	n := len(bands)
	for i := 0; i < n/2; i++ {
	...
	// are faint warm-grey context.
	for _, s := range samples {
		color, width, op := "#9C8C7C", "1", "0.55"
		if len(s) > 0 && s[len(s)-1] <= 0 {
			color, width, op = "#C0492F", "1.8", "0.9"
		}
	...
	if n%2 == 1 {
		fmt.Fprintf(&b, `<path d="%s" fill="none" stroke="#2A231F" stroke-width="2.2" stroke-linejoin="round"/>`+"\n",
