...
func Fan(opt Options, xLabel string, bands [][]float64, samples [][]float64) string {
	...
	for _, i := range axisTicks(steps) {
		...
		fmt.Fprintf(&b, `<text x="%.1f" y="%.1f" font-size="12" fill="#55666E" text-anchor="middle">%d</text>`+"\n", xAt(i), y1+16, i)
	}
	if xLabel != "" {
	...
	if n%2 == 1 {
		fmt.Fprintf(&b, `<path d="%s" fill="none" stroke="#14232B" stroke-width="2.2" stroke-linejoin="round"/>`+"\n",
