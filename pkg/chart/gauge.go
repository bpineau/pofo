...
func Gauge(opt Options, value, caption, left, right string, frac float64) string {
	...
	fmt.Fprintf(&b, `<defs><linearGradient id="gaugeg" x1="0" y1="0" x2="1" y2="0">`+
		`<stop offset="0" stop-color="`+themeGood+`"/><stop offset="0.5" stop-color="`+themeWarn+`"/><stop offset="1" stop-color="`+themeBad+`"/></linearGradient></defs>`+"\n")
	// Track then the coloured arc.
	fmt.Fprintf(&b, `<path d="%s" fill="none" stroke="`+themeWell+`" stroke-width="18" stroke-linecap="round"/>`+"\n", arc(r))
	fmt.Fprintf(&b, `<path d="%s" fill="none" stroke="url(#gaugeg)" stroke-width="18" stroke-linecap="round"/>`+"\n", arc(r))

	// Needle.
	nx, ny := pt(frac, r-4)
	fmt.Fprintf(&b, `<line x1="%.1f" y1="%.1f" x2="%.1f" y2="%.1f" stroke="`+themeInk+`" stroke-width="3" stroke-linecap="round"/>`+"\n", cx, cy, nx, ny)
	fmt.Fprintf(&b, `<circle cx="%.1f" cy="%.1f" r="5.5" fill="`+themeInk+`"/>`+"\n", cx, cy)

	// Central value and caption.
	fmt.Fprintf(&b, `<text x="%.1f" y="%.1f" text-anchor="middle" font-family="'Spline Sans Mono',ui-monospace,monospace" font-size="30" font-weight="600" fill="`+themeInk+`">%s</text>`+"\n", cx, cy-16, esc(value))
	if caption != "" {
		fmt.Fprintf(&b, `<text x="%.1f" y="%.1f" text-anchor="middle" font-size="12" fill="`+themeInkSoft+`">%s</text>`+"\n", cx, cy+4, esc(caption))
	}
	lx, ly := pt(0, r)
	rx, ry := pt(1, r)
