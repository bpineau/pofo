...
func MultiLine(opt Options, xLabel, yLabel string, series []XYSeries, markers ...Marker) string {
	...
	if yLabel != "" {
		fmt.Fprintf(&b, `<text x="14" y="%.1f" font-size="12" fill="#55666E" text-anchor="middle" transform="rotate(-90 14 %.1f)">%s</text>`+"\n", (y0+y1)/2, (y0+y1)/2, esc(yLabel))
	}
	// Axes.
	...
	for _, m := range markers {
		...
		case 'x':
			x := xAt(m.Value)
			fmt.Fprintf(&b, `<line x1="%.1f" y1="%g" x2="%.1f" y2="%g" stroke="#8A979C" stroke-dasharray="4 3"/>`+"\n", x, y0, x, y1)
			fmt.Fprintf(&b, `<text x="%.1f" y="%g" font-size="11" fill="#8A979C" text-anchor="middle">%s</text>`+"\n", x, y0-2, esc(m.Label))
		case 'y':
			y := yAt(m.Value)
			fmt.Fprintf(&b, `<line x1="%g" y1="%.1f" x2="%g" y2="%.1f" stroke="#8A979C" stroke-dasharray="4 3"/>`+"\n", x0, y, x1, y)
			fmt.Fprintf(&b, `<text x="%g" y="%.1f" dy="-2" font-size="11" fill="#8A979C" text-anchor="end">%s</text>`+"\n", x1, y, esc(m.Label))
		}
	}
