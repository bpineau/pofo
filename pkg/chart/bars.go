...
func Bars(opt Options, bars []Bar) string {
	...
	for v := 0.0; v <= max+step/1e6; v += step {
		y := yAt(v)
		fmt.Fprintf(&sb, `<line x1="%d" y1="%.1f" x2="%d" y2="%.1f" stroke="#E8ECEA"/>`, padL, y, w-padR, y)
		fmt.Fprintf(&sb, `<text x="%d" y="%.1f" dy="0.35em" font-size="12" fill="#55666E" text-anchor="end">%s</text>`, padL-6, y, fmtTick(v, step))
	}
	...
	for i, b := range bars {
		...
		if b.Text != "" {
			fmt.Fprintf(&sb, `<text x="%.1f" y="%.1f" text-anchor="middle" font-size="12" fill="#14232B">%s</text>`, x+bw/2, y-4, esc(b.Text))
		}
