// HTML report assembly: the Page model (HTMLPage), the statistics table
...
// as the CLI does.
func (c *Comparison) HTMLPage(d Decoration) *report.Page {
	results := c.columns
	bench := c.bench
	commonStart, commonEnd := c.commonStart, c.commonEnd
	meta := c.meta
	...
	page := &report.Page{
		...
		RebalanceDays:  c.opt.Rebalance,
		CommonStart:    commonStart.Format("2006-01-02"),
		CommonEnd:      commonEnd.Format("2006-01-02"),
		PortfolioNames: names,
	}
	// Presentation chrome from the Decoration: a skin stylesheet, the site nav
	// and the composer widget. The zero Decoration leaves them empty, which
	// report.Page treats as "omit", so the standalone report is unchanged.
	page.SkinCSS = d.SkinCSS
	page.SiteNav = d.SiteNav
	page.Composer = d.Composer
	...
	for _, r := range results {
		...
		if r.rebalanceDays != c.opt.Rebalance {
		...
		if len(section.Coverage) > 0 {
			...
			if c.opt.Framework.Name == "factors" {
...
// StatRows renders the comparison's statistics table: one row per metric, each
// cell holding a portfolio's value and marked when it wins the row.
func (c *Comparison) StatRows() []report.StatRow {
	return buildStatRows(c.columns, c.opt.Benchmark)
}

// CoverageBars computes the macro-regime coverage bars for one portfolio's
// holdings, under the comparison's suggestion framework.
func (c *Comparison) CoverageBars(assets []portfolio.Asset) []report.CoverageBar {
	return coverageBars(assets, c.meta, c.opt.Framework)
}

func buildStatRows(results []*column, benchmark string) []report.StatRow {
	// cell computes a row's value (for the best-of-row comparison) and its
	// display text for one portfolio.
	type def struct {
		label  string
		hint   string
		cell   func(r *column) (float64, string)
		better int // +1 higher wins, -1 lower wins, 0 no highlight
	}
	pct := func(get func(metrics.Stats) float64) func(*column) (float64, string) {
		return func(r *column) (float64, string) { v := get(r.stats); return v, fmtPct(v) }
	}
	num := func(get func(metrics.Stats) float64) func(*column) (float64, string) {
		return func(r *column) (float64, string) { v := get(r.stats); return v, fmtNum(v) }
	}
	...
	money := func(get func(r *column) (float64, bool)) func(*column) (float64, string) {
		return func(r *column) (float64, string) {
			if r.p.Capital <= 0 {
				return math.NaN(), "-"
			}
			v, ok := get(r)
	...
	moneyDefs := []def{
		{"Starting capital", "from \"#meta capital:\"",
			money(func(r *column) (float64, bool) { return r.p.Capital, true }), 0},
		{"Total contributed", "external money added over the whole simulated span",
			money(func(r *column) (float64, bool) { return r.sim.Contributed, true }), 0},
		{"Total withdrawn", "money taken out over the whole simulated span",
			money(func(r *column) (float64, bool) { return r.sim.Withdrawn, true }), 0},
		{"Final value", "worth at the end of the simulated span, flows included",
			money(func(r *column) (float64, bool) { return r.sim.Values[len(r.sim.Values)-1], true }), 0},
	...
	defs := []def{
		...
		{"TTR (longest recovery)", "duration of the longest stretch below a peak",
			func(r *column) (float64, string) { return float64(r.stats.TTRDays), fmtTTR(r.stats) }, -1},
