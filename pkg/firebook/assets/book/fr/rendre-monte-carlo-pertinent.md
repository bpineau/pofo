...
Ces six corrections forment aussi une grille d'évaluation. Devant un simulateur, la bonne question n'est pas combien de trajectoires il tire, mais lesquelles de ces corrections il applique, et lesquelles il laisse de côté sans le dire.
...
**Correction 5 : garder les données en juges.** Toutes les corrections précédentes raffinent un modèle paramétrique, et le risque ultime serait de ne plus regarder que lui. D'où la règle : garder en permanence sous les yeux les modèles de données (fenêtres historiques, bootstrap de vos fonds, broad-sample du siècle), qui ne dépendent d'aucun paramètre saisi ([[historique-vs-parametrique]]). Ils jouent le rôle de la réalité dans la pièce. Si le central corrigé s'écarte beaucoup des fenêtres historiques, l'écart rend visible le biais de votre fenêtre. S'il s'écarte du broad-sample, c'est votre pari sur « le futur sera plus doux que le siècle » qui devient explicite. Le désaccord entre modèles n'est pas un défaut à résoudre, c'est le produit fini ([[ruine-et-probabilites]]). Si votre outil ne sait afficher qu'un modèle à la fois, reconstituez ce contradictoire à la main, en rejouant le même plan dans un rejeu historique (cFIREsim, FICalc) et dans un bootstrap (Portfolio Visualizer).
...
- Le produit fini n'est pas un chiffre mais un faisceau : un central honnête encadré de bornes ; on décide dans le faisceau, sur les modèles durs.
...
- Chaque correction a trente ans de littérature derrière elle. La difficulté n'est pas d'en trouver une bonne, c'est de les appliquer **toutes**, par défaut, sans en réserver la moitié à qui saura les demander.
...
- Dans ce livre : [[la-machine-pofo]] et [[utiliser-la-page-fire]], pour ces six corrections telles que pofo les implémente.
