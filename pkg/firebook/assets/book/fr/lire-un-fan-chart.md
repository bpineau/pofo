...
**Les chemins d'exemple.** Les deux trajectoires fines (verte et rouge sur la figure) sont des futurs individuels, et leur choix est une astuce de conception qui mérite d'être comprise. Les bons fan charts les prélèvent à rangs **régulièrement** espacés de richesse finale (la pire, puis le pire septième, le pire quart... jusqu'à la meilleure). Conséquence directe : **le nombre de chemins rouges (échoués) encode le taux d'échec**. Un rouge sur huit chemins tracés ≈ « le pire septième échoue » (ruine ≤ ~14 %) ; deux rouges ≈ un futur sur sept ; trois ≈ un sur quatre. Un coup d'œil aux rouges donne l'ordre de grandeur de la ruine sans lire un seul chiffre.
