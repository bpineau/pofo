...
La brique se teste avant de s'acheter, à condition d'un simulateur dont le catalogue contient des historiques reconstruits de fonds efficient core et de stratégies empilées actions + trend (pofo en embarque, calibrés sur les indices de référence). Composez votre plan actuel contre sa version « cœur levé + trend libéré » et lisez les trois mêmes juges que d'habitude → le central (qui doit monter un peu), le stress 1966/1973 (qui doit s'améliorer nettement), et le drawdown réel maximal (qui dit le prix psychologique, [[lire-un-fan-chart]]).
:::
