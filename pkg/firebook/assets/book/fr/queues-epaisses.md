...
| 5 (le défaut raisonnable) | Le monde des données mensuelles réelles | ~1 sur 40 | Une à deux fois |
...
- La Student-t ajoute le paramètre manquant : df, l'épaisseur des queues. À σ identique, df 5 rend l'année à −30 % réel ~10 fois plus probable que df 30. Ce df s'estime en inversant la relation kurtosis ≈ 3 + 6/(df − 4) sur les rendements mensuels de **vos** fonds, ce qui donne typiquement 4-6.
...
- Limites du modèle : symétrique (le vrai risque a un skew négatif, couvert par les stress de séquence et la décennie perdue) et sans mémoire (couvert par les régimes). La diversification protège moins dans la queue, sauf les briques dont la décorrélation survit aux crises.
- Réflexe pratique : gardez le df estimé sur vos données, faites une fois le test 3/30 pour connaître votre exposition, et si la décision bascule, réparez par la structure du plan, pas par le paramètre.
...
- L'estimation du df et son usage exact dans pofo : [[la-machine-pofo]], et [[utiliser-la-page-fire]] pour le mode d'emploi.
