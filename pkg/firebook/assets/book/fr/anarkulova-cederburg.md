...
**La méthode.** Plutôt que rejouer des fenêtres d'un seul pays, un **bootstrap par blocs**. On tire des blocs de dix ans dans l'ensemble pays × époques, pour préserver les grappes, tendances et régimes ([[sequence-des-rendements]]). On assemble ensuite des retraites synthétiques de la durée voulue. Chaque retraite simulée vit donc l'histoire d'un pays développé cohérent, morceau par morceau, catastrophes comprises. C'est très exactement ce que fait le modèle « broad sample » du simulateur FIRE de pofo, construit sur le panel académique Jorda-Schularick-Taylor (16 pays, 1870-2020) et rejoué en blocs par pays sur un portefeuille 60/40 domestique ([[la-machine-pofo]]).
...
Plan : 1,4 M€, 45 000 €/an rigides (3,2 %), 45 ans, pension 15 000 €/an à 67 ans, actions mondiales 70 % / obligations 30 %. Fenêtres historiques du portefeuille → ruine ~1 %. Central calibré → ~4 %. Broad-sample → ~9 %, échecs concentrés dans les blocs à inflation persistante, aux trois quarts après 80 ans, pension acquise. Lecture : le plan est solide, car 3,2 % avec pension est déjà prudent. La queue broad-sample se traite par 10-15 % de flexibilité écrite ([[flexibilite-realite]]) plutôt que par du capital en plus. Sans la colonne broad-sample, on n'aurait jamais su que le mode de défaillance résiduel était l'inflation, pas le krach.
:::
