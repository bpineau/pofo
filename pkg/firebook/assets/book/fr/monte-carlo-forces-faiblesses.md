...
Puis recommencer quelques milliers de fois et compter. La fraction ruinée donne la probabilité de ruine. Les richesses année par année donnent le cône ([[lire-un-fan-chart]]). Les dépenses servies donnent la distribution du niveau de vie réellement vécu, et ainsi de suite. Il n'y a rien d'autre dans la boîte, juste de la comptabilité exacte appliquée à des futurs tirés au sort. (Le détail de cette boucle telle que pofo l'implémente est dans [[la-machine-pofo]].)

## Ce que Monte-Carlo fait mieux que tout le reste

Pour apprécier l'outil, comparons-le à ses trois concurrents.

**Contre le rejeu historique pur** (la méthode Bengen, [[etude-trinity]]). L'histoire américaine ne contient qu'une centaine d'années, et les fenêtres de 30 ans qu'on y découpe se recouvrent presque toutes : cela ne fait que trois ou quatre retraites réellement indépendantes. Le rejeu répond à « qu'aurait donné mon plan dans le passé ? ». Il ne peut, par construction, rien dire des futurs qui ne ressemblent à aucun passé. Et il traite le pire millésime historique comme une borne, alors que rien ne garantit que le pire soit déjà arrivé. Monte-Carlo génère, lui, des dizaines de milliers d'années synthétiques. Il explore l'espace des possibles autour des hypothèses, pas seulement le chemin réalisé. Les deux sont complémentaires. C'est pourquoi les bons simulateurs affichent les deux côte à côte.
...
Le même plan, avec un seul réglage modifié à chaque fois. En haut, on déplace μ d'un demi-point vers le haut puis vers le bas. La ruine passe de 4,1 % à 8,7 %, un facteur 2,1, alors qu'aucune série de données ne permet de trancher entre ces trois valeurs. En bas, on multiplie le nombre de trajectoires par dix, hypothèses figées. Le résultat ne bouge pas ; seule la barre d'incertitude se raccourcit, de ±1,5 point à 1 000 tirages à ±0,5 point à 10 000. **Toute la précision utile est du côté des entrées.**
:::
...
**2. Ne lisez jamais un seul modèle.** L'intervalle entre les modèles (central, stress de séquence, échantillon mondial, historique) est l'information. Un modèle seul n'est qu'une opinion ([[ruine-et-probabilites]]).
...
**6. Simulez vos règles réelles, pas la caricature rigide.** Regardez ensuite le niveau de vie servi, pas seulement la survie. Une règle flexible « réussit » toujours au sens de la ruine. La vraie question devient ce qu'elle vous fait vivre ([[flexibilite-realite]]).
