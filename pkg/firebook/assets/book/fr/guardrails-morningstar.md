...
::: admin Mode d'emploi
- **Le capteur ne s'applique plus à un ratio, mais au plan entier.** Deux formulations équivalentes existent, et il faut savoir qu'elles le sont. La première : relancer le simulateur aux chiffres du jour et lire la probabilité de succès. La seconde : calculer la dépense de l'année divisée par la **richesse totale** (portefeuille + valeur actualisée des pensions à venir), et la comparer non pas au taux de départ, mais au taux encore **sûr pour l'horizon restant**, tabulé une fois pour toutes sous vos hypothèses. Les deux disent la même chose ; la seconde coûte mille fois moins cher à calculer, ce qui est la façon dont les outils l'implémentent réellement.
...
- **De tête.** Impossible, et c'est la limite assumée de la méthode. Elle exige un simulateur, ou au minimum une table du taux sûr pré-calculée. C'est très exactement le prix du meilleur capteur.
:::
...
- Dans ce livre : [[guyton-klinger]] (l'ancêtre et ses quatre paramètres), [[plancher-plafond]] (le corridor continu, l'autre école du revenu piloté), [[choisir-sa-strategie]] (l'arbitrage final). Pour conduire les deux capteurs pas à pas dans pofo, voir [[utiliser-la-page-fire]].
