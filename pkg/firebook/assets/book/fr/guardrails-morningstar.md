...
**La version native**, pour qui veut la règle simulée en continu plutôt que la procédure annuelle. La case « Risk-based guardrails » applique le capteur par risque à l'intérieur même de la simulation. La bande ne suit plus le taux de départ mais le taux encore sûr pour l'horizon restant, tabulé une fois pour toutes sous vos hypothèses centrales, et le taux courant se lit sur la richesse totale, pensions à venir actualisées comprises. Les ajustements restent ceux de la famille, ±10 % à chaque franchissement, avec le même curseur de plancher que la version 2006 ([[guyton-klinger]]). Deux nuances d'implémentation méritent d'être connues. Le capteur est exprimé en taux de retrait plutôt qu'en probabilité de succès recalculée chaque année, ce qui revient au même signal pour un coût de calcul sans commune mesure, l'un étant la lecture de l'autre. Et la table du taux sûr est résolue sous le modèle que vous avez sélectionné dans le bandeau, puis conservée telle quelle : une table écrite sur un historique flatteur autorisera des hausses qu'une colonne dure sanctionnera ensuite, et cet écart est précisément le risque de modèle dont parle le paragraphe précédent. Restent la §04 et la frontière §06 pour voir ce que votre version ferait vivre. Réglez la règle sur vos paramètres écrits, puis regardez le pire quartile avant de signer.
...
- Dans le simulateur FIRE de pofo : la case « Risk-based guardrails » (la règle en natif, bande sur le taux sûr de l'horizon restant), le solveur §09 (dimensionner les seuils), la §04 (le pire quartile vécu), la frontière §06, où la famille apparaît deux fois, capteur 2006 et capteur par risque ([[utiliser-la-page-fire]]).
