...
La théorie de la décision, un siècle de travaux entre économie, mathématiques et psychologie, a des réponses précises à cette question. Vous allez voir pourquoi l'espérance de gain est un mauvais critère (l'utilité), comment comparer deux plans risqués en un seul chiffre honnête (l'équivalent certain), pourquoi le critère de Kelly, si séduisant, ne doit **pas** être appliqué tel quel à une retraite, ce que valent le regret et la robustesse comme critères d'adulte, et comment tout cela se condense en un protocole de décision en cinq règles. C'est l'article le plus conceptuel du livre, et peut-être le plus rentable : les erreurs de critère coûtent plus cher que les erreurs de calcul.
...
Le point de départ classique est un paradoxe de 1738, celui de Saint-Pétersbourg : un jeu de pile ou face à espérance de gain **infinie** que personne ne paierait plus de quelques dizaines d'euros pour jouer. La résolution de Daniel Bernoulli fonde toute la suite : ce qui compte n'est pas l'argent mais l'**utilité** de l'argent, et celle-ci croît de moins en moins vite. Passer de 500 000 € à 1 M€ change une vie ; passer de 10 M€ à 10,5 M€ ne change rien. Cette **concavité** a une conséquence directe : à espérance égale, moins de dispersion vaut plus d'utilité, l'aversion au risque n'est pas une émotion mais un théorème.

Pour le rentier, la concavité est extrême et asymétrique. La zone basse de sa distribution (les trajectoires qui frôlent ou touchent la ruine, [[ruine-et-probabilites]]) détruit le mode de vie, la dignité, les options ; la zone haute (mourir sur un tas d'or) n'ajoute presque rien qu'un héritage plus gros ([[depenses-en-retraite]]). Un critère de décision qui pèse ces deux zones symétriquement, comme la moyenne, est donc structurellement faux pour lui. C'est la justification profonde d'une habitude que ce livre a partout : juger les plans sur le **percentile 5** et la médiane, jamais sur la moyenne ([[lire-un-fan-chart]]).
...
Le critère de Kelly (1956) répond élégamment à une vraie question : quelle fraction de son capital risquer sur un pari favorable pour maximiser la **croissance géométrique** à long terme ([[rendements-arithmetiques-geometriques]]) ? Appliqué aux marchés, il recommande typiquement des expositions actions de 120 à 200 %, et il a trois défauts rédhibitoires pour une retraite. Il suppose un horizon infini, alors que le rentier a 30-50 ans et un besoin de consommation daté. Il est indifférent aux drawdowns, or le Kelly complet traverse sereinement des chutes de 50 % (probabilité 1/2 d'y passer un jour) et de 90 % (1/10), que le retrait transforme en ruine par risque de séquence ([[sequence-des-rendements]]). Et il exige de connaître les vrais paramètres, alors qu'une petite erreur d'estimation en sur-Kelly détruit la croissance au lieu de la maximiser, l'erreur est asymétrique. La leçon utile à en retenir est le **Kelly fractionnaire** : les praticiens sérieux misent un demi-Kelly ou moins, sacrifiant peu de croissance contre beaucoup de tranquillité ; la position actions d'un plan de retrait bien construit se retrouve, sans l'avoir cherché, quelque part vers le quart ou le tiers de Kelly. Si un produit ou un blog vous vend du Kelly complet pour votre retraite, il a lu la formule et pas les hypothèses.
:::
...
**Le regret d'abord.** Harry Markowitz lui-même, inventeur de l'optimisation de portefeuille, racontait avoir placé sa propre épargne en 50/50 « pour minimiser mon regret futur » plutôt qu'à l'optimum de sa théorie. Le critère minimax regret (choisir l'option dont le pire regret rétrospectif est le plus petit) est parfaitement rationnel quand on ne vit qu'une trajectoire : il explique pourquoi garder une part d'actions dans un plan prudent (le regret de rater trente ans de hausse) ET une part d'obligations dans un plan agressif (le regret du krach de l'an 1) domine les solutions en coin. Beaucoup de choix « tièdes » de ce livre, la dose d'or, le corridor de retrait, le 60/40 élargi, sont des minimisations de regret assumées, et c'est un compliment.

**Le satisficing ensuite** (Herbert Simon) : chercher une solution **suffisante** plutôt que la meilleure. Un plan qui atteint 95 % de réussite avec un revenu qui vous suffit n'a rien à gagner à l'optimisation supplémentaire, mais beaucoup à y perdre en complexité, en frais et en fragilité. La question de Simon (« assez bon pour quoi ? ») est plus utile au rentier que celle de l'optimiseur (« maximal selon quel modèle ? »), car la seconde a toujours pour vraie réponse « selon un modèle faux » ([[monte-carlo-forces-faiblesses]]).

**La robustesse enfin.** Une décision robuste est une décision qui reste bonne quand le modèle qui l'a produite se trompe. Les optima de portefeuille sont notoirement des crêtes fragiles (changer le rendement attendu d'un point déplace l'allocation « optimale » de vingt), mais la nature offre une grâce : autour de l'optimum, la surface est **plate**. Entre 50 et 80 % d'actions, le SWR bouge à peine ([[allocation-actions-obligations]]) ; entre 5 et 15 % d'or ou de trend, idem. La bonne pratique découle du relief : identifier le plateau, s'installer **au milieu** du plateau (pas au bord, là où une erreur de modèle vous fait basculer), et fuir toute recommandation qui n'existe que sur une crête. C'est l'idée de l'optimisation robuste, version praticable sans mathématiques : optimiser contre la pire des hypothèses plausibles, pas pour la meilleure des estimations.
...
- La décision obéit au minimum de deux curseurs → tolérance (psychologique, découverte dans les baisses) et capacité (objective, calculée par le simulateur).
