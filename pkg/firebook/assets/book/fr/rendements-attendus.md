...
Le rendement passé d'un actif n'est pas son espérance. C'est même souvent, en partie, son **inverse**. Une décennie exceptionnelle est en grande partie une expansion de valorisation, c'est-à-dire du rendement futur déjà consommé. À l'inverse, les obligations affichent leur espérance sur l'étiquette (le rendement à l'échéance), et les actions l'affichent à moitié (l'earnings yield, les bénéfices rapportés au prix, [[valorisations-et-cape]]). Estimer prospectivement, ce n'est pas prédire. C'est lire ce qui est déjà écrit dans les prix.
:::
...
Portefeuille : 70 % actions mondiales (dont ~65 % US), 30 % obligations euro aggregate. Actions : pondérons 0,65 × 3 % (US, hypothèse médiane) + 0,35 × 5 % (reste du monde) ≈ 3,7 % réel. Obligations : YTM ~3,2 %, inflation anticipée ~2 % → 1,2 % réel. Portefeuille : 0,7 × 3,7 + 0,3 × 1,2 ≈ **2,95 % réel géométrique attendu**. Le paramètre μ d'un modèle paramétrique se saisit en moyenne **arithmétique** ([[rendements-arithmetiques-geometriques]]) : ajoutez σ²/2, soit ~0,6 point à σ = 11 % → **μ ≈ 3,5 %**. Comparez ce chiffre à la valeur par défaut de votre outil, et à ce que suggère votre historique de fonds. L'écart est le débat, et il vaut mieux le trancher consciemment que le subir.
:::
...
**Oublier que le futur peut être pire que toutes les prévisions.** Les CMA sont des espérances centrales : aucune ne « contient » 1929, le Japon de 1990 ou une guerre. C'est le rôle des autres modèles (stress de séquence, décennie perdue, siècle mondial) et des protections structurelles ([[portefeuilles-tous-temps]], [[flexibilite-realite]]). L'espérance prospective calibre le centre. Elle ne remplace ni les queues ni les marges.
...
- Morningstar recalcule chaque année le taux de retrait recommandé sur ces bases (3,3 → 3,8 → 4,0 → 3,7 → 3,9 % de 2021 à 2025) : la preuve vivante que le taux soutenable dépend des conditions d'entrée, et de la méthode qui les traduit.
...
- Dans un simulateur : une seule calibration centrale (blending par défaut, building-blocks manuel, ou ancre CAPE), les autres lectures servant de bornes. N'empilez pas trois fois la même prudence, elle se paie en années de travail.
