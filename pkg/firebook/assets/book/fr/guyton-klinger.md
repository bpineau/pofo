...
Pour un plan FIRE long qui retient la famille guardrails version 2006, choisie pour sa simplicité de calcul à la main là où le guardrail par risque demande un simulateur (le simulateur FIRE de pofo en a un en natif, [[guardrails-morningstar]]), voici la configuration que soutient la littérature post-ERN :
...
- Dans le simulateur FIRE de pofo : case guardrails (corridor ±20 %, ajustements ±10 %) plus curseur de plancher, et juste à côté la version par risque si vous préférez le bon capteur ([[guardrails-morningstar]]). Jugez toujours sur la §04 (la vie vécue) et la frontière §06, jamais sur la seule ruine.
