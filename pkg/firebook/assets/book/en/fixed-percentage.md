...
- **Indexation.** Nothing to index, and this is a classic implementation trap: if you compute on the portfolio in current dollars, inflation is already in the answer, and indexing again would count it twice. The Yale rule is the exception, since its memory term does have to be re-indexed.
