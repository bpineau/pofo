...
//
// It is a thin wrapper over bookmd.ToHTML wired with the edition's figure
// renderer and callout labels; the shared Markdown dialect lives in
// pkg/bookmd.
func (e *Edition) ToHTML(src string, titles map[string]string) string {
	return bookmd.ToHTML(src, bookmd.Options{Titles: titles, Figure: e.Figure, Callouts: e.Callouts})
}
