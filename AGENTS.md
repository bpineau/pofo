...
| `cmd/pofo` | CLI wiring only, one file per concern: `main.go` (flags + mode dispatch + terminal output), `fetch.go`, `page.go` (HTML report assembly), `composition.go` (pies/coverage), `contrib.go` (contribution charts), `suggest.go`, `simdata.go`, `optimize.go`, `fire.go`, `permanent.go`; the `-serve` web constellation is `serve.go` (mux + lifecycle), `hub.go` (the front-door catalog page), `view.go` (the shareable `/view` URL grammar) and `prefs.go` (the settings cookie) |
