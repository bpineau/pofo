...
make tyield-refdata # regenerate the US Treasury references: TREASURY-LONG-YIELD (the long par yield the STRIPS reconstruction is priced off) and the month-end total returns TREASURY-LONG-USD / TREASURY-INT-USD (network); run make simdata after
...
| `pkg/permanent` | tactical Permanent Portfolio 2.0 (Darcet): reads `datasets.MacroPanel` into a growth×inflation + monetary regime, quadratically-damped four-sleeve allocation, monthly-real backtest, coarse `Regime.Quadrant` view (used by the report's regime strip), and the publication-honest information set a backtest owes its signal (`SignalConfig.ReleaseLags`, `PublicationLags()`: NOT the default, since a regime is also read descriptively); see `docs/darcet-permanent-portfolio-design.md` |
...
| `pkg/simgen` | rebuilds the missing past of complex assets (constant-weight composites, cap-weighted drifting blends (`CapWeighted`), TSMOM engine, donor chains) into simdata files; `audit.go` grades every engine against the real quotes (`Audit`/`AuditAll`, behind `pofo -verify-simdata`) |
...
- Employee-savings fund (FCPE: `ERESMONDEM`, the world-equity fund, and
  ...
  FY2025 report, never the measured residual.
